(ns shoko.cacao-test
  ...
  well-formed CBOR envelope. The per-actor key model is the shoko/koyomi/
  teian/kekkai analog — the key IS the identity."
...
(deftest minted-cacao-signature-verifies
  (testing "the Ed25519 signature is over the exact SIWE message"
    (let [id    (c/generate-identity)
          grant {:cap :cap/read :scope "shoko"}
          opts  {:aud "https://kotobase.net" :nonce "n1" :issued-at "2026-07-07T00:00:00Z"}
...
(deftest minted-cacao-is-wellformed-cbor
  (let [id    (c/generate-identity)
        cacao (c/mint id {:cap :cap/transact :scope "shoko"}
                      {:aud "https://kotobase.net" :nonce "n2"
                       :issued-at "2026-07-07T00:00:00Z" :expiry "2026-07-07T01:00:00Z"})
...
(deftest per-actor-key-persists
  (testing "load-or-create! generates once, then reloads the same identity"
    (let [p  (str (System/getProperty "java.io.tmpdir") "/shoko-id-test-" (System/nanoTime) ".edn")
...
(deftest resources-encode-capability-and-graph
  (is (= ["kotoba://op/datom:read" "kotoba://graph/shoko"]
         (c/grant->resources {:cap :cap/read :scope "shoko"})))
