(ns shoko.kotoba
  ...
  itself is already proven backend-agnostic (test/shoko/store_contract_test)."
  (:require [clojure.string :as str]
            [langchain.kotoba-db :as kdb]
            [shoko.cacao :as cacao]
...
(defn kotoba-store
  "A `store/DatomicStore` backed by a kotoba-server pod (e.g. kotobase.net).
   Auth options (pick one):
     ...
     :identity         a shoko.cacao identity ({:private-key :public-key :did})
